...
module TreeNodes
  ...
  module ClassMethods
    ...
    def calculate_tree_positions(objs)
      ...
      objs.map{|obj| [obj.id, ([obj.id] + ancestors.fetch(obj.id, [])).reverse.map{|node_id| positions.fetch(node_id).to_s(16).rjust(16, '0')}.join]}.to_h
    end
